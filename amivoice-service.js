...
class AmiVoiceService extends EventEmitter {
  ...
  connect() {
    if (!this.isReady) {
      ...
      client.on("connectFailed", (error) => {
        log("AmiVoice Service: " + error.toString());
        this.isReady = false;
      });

      client.on("connect", (connection) => {
        this.connection = connection;
        log("AmiVoice Service: websocket client connected");
        ...
        connection.on("message", (message) => {
          if (message.type === "utf8") {
            ...
            switch(code) {
              case "s":
                log("AmiVoice Service: [s] Connection established");
                this.isStarted = true
                break;
              case "A":
                log(`AmiVoice Service: [A] ${data.text}`);
                this.emit("transcription", { body: data });
                break;
              case "e":
                log("AmiVoice Service: [e] Connection closed");
        ...
        connection.on("close", () => {
          this.isStarted = false;
          log("AmiVoice Service: connection closed");
        });
  ...
  close() {
    if (this.isStarted && this.isReady) {
      log("AmiVoice Service: send close command");
      const endCommand = "e";
